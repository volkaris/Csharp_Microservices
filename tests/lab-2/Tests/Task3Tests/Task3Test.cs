...
public class Task3Test
{
    ...
    public async Task TestFunctionality()
    {
        ...
        var configurationServiceProvider = new ConfigurationService(provider, serviceProvider.GetRequiredService<IConfigurationClient>());

        using var cancellationTokenSource = new CancellationTokenSource();

        cancellationTokenSource.CancelAfter(TimeSpan.FromSeconds(3));

        await configurationServiceProvider.UpdateAfter(4, null, TimeSpan.FromSeconds(1), cancellationTokenSource.Token);
        ...
        await ordersService.DeleteProductsFromOrder(order, product1, CancellationToken.None);

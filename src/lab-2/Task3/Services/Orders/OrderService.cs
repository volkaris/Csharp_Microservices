...
public class OrderService : IOrderService
{
    ...
    public async Task DeleteProductsFromOrder(OrderDto order, ProductDto product, CancellationToken cancellationToken)
    {
        using var transaction = new TransactionScope(
            TransactionScopeOption.Required,
            new TransactionOptions() { IsolationLevel = IsolationLevel.Serializable },
    ...
    private async Task ChangeStatus(OrderDto order, OrderState newState, CancellationToken cancellationToken)
    {
        ...
        await _orderRepository.ChangeOrderState(order.ToOrder(), newState, cancellationToken);
